...
RUN cargo build --release --package echo-service
...
RUN touch libs/common/src/lib.rs services/echo-service/src/main.rs && \
    cargo build --release --package echo-service
...
COPY --from=builder /app/target/release/echo-service /app/echo-service
